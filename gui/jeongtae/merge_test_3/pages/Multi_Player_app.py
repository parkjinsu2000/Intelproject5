...
from PyQt5.QtCore import QTimer, Qt, QRect, QPropertyAnimation, QEasingCurve, QCoreApplication, pyqtSignal
...
class MultiPlayerApp(BasePoseApp):
    ...
    def __init__(self, args, model, use_half, user_name, user_id, video_title):
        ...
        self.local_scores = collections.defaultdict(lambda: 100)
            ...
            if tid in tracked_players:
                ...
                if cam_kps is not None and cam_kps.size > 0 and len(self.reference_data) > 0:
                    ...
                    if ref_data_index < len(self.reference_data):
                        ...
                        # `pose_utils.py`의 `ANGLE_TRIPLES` 순서에 맞춰 가중치를 정의합니다.
                        # 0: 왼팔, 1: 오른팔, 2: 왼다리, 3: 오른다리
                        # 4: 왼쪽 몸통, 5: 오른쪽 몸통
                        
                        num_angles = len(vec_ref)
                        weights = np.ones(num_angles)
                        
                        # 팔과 다리 각도에 더 높은 가중치 부여 (예: 2.0)
                        weights[0:4] = 2.0
                        
                        vec_ref_weighted = vec_ref * weights
                        vec_live_weighted = vec_live * weights

                        # 가중치가 적용된 벡터를 사용하여 점수 계산
    ...
    def display_final_score(self):
        ...
        if self.button_container is None:
            ...
            self.rank_btn.setStyleSheet("background-color: white; padding: 10px; border-radius: 10px;")
            layout.addStretch(1)
