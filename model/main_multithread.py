...
def normalize_keypoints(pts):
    ...
    if not np.any(np.isfinite(center)):
        center = np.nanmean(pts[np.isfinite(pts).all(axis=1)], axis=0)
    if not np.any(np.isfinite(center)):
        return pts # Cannot normalize
    out = pts - center
    # 스케일: 어깨폭(없으면 분산 반경)
    if np.any(~np.isfinite(pts[[L_SH, R_SH]])):
        finite = pts[np.isfinite(pts).all(axis=1)]
        scale = np.max(norm(finite - np.nanmean(finite, axis=0), axis=1)) if len(finite) > 1 else 1.0
    else:
...
# ---------------- 백그라운드 Pose Worker ----------------
def pose_worker(q_in, q_out, infer_func):
    while True:
        frame_data = q_in.get()
        ...
        try:
            kps, conf = infer_func(frame)
            q_out.put((frame_idx, kps, conf))
        except Exception as e:
            print(f"[error] Pose worker failed: {e}")
            q_out.put((frame_idx, None, None)) # Put a failure result
            continue
...
def main():
    ...
    # --- 스레드 및 큐 설정 ---
    q_in_ref = Queue(maxsize=1)
    q_out_ref = Queue(maxsize=1)
    q_in_user = Queue(maxsize=1)
    q_out_user = Queue(maxsize=1)

    worker_ref = Thread(target=pose_worker, args=(q_in_ref, q_out_ref, infer_pose), daemon=True)
    worker_user = Thread(target=pose_worker, args=(q_in_user, q_out_user, infer_pose), daemon=True)
    worker_ref.start()
    worker_user.start()
    ...
    if not okR0: raise RuntimeError("Reference video has no frames.")
    ...
    while True:
        ...
        def draw_countdown_overlay(img, t):
            H, W = img.shape[:2]
            scale = max(1.0, min(H, W) / 180.0)
            thickness = max(2, int(scale * 4))
            (tw, th), _ = cv2.getTextSize(t, cv2.FONT_HERSHEY_SIMPLEX, scale, thickness)
            x = (W - tw) // 2; y = (H + th) // 2
            cv2.putText(img, t, (x, y), cv2.FONT_HERSHEY_SIMPLEX, scale, (0,0,0), thickness+2, cv2.LINE_AA)
            cv2.putText(img, t, (x, y), cv2.FONT_HERSHEY_SIMPLEX, scale, (255,255,255), thickness, cv2.LINE_AA)
        ...
        if (cv2.waitKey(1) & 0xFF) == ord('q'):
            ...
            q_in_ref.put(None); q_in_user.put(None)
            return
    ...
    while True:
        ...
        if do_score:
            # 워커에게 프레임 전송 (논블로킹)
            try:
                q_in_ref.put_nowait((frame_idx, fR))
            except Full:
                pass # 워커가 바쁘면 프레임을 건너뜀
            try:
                q_in_user.put_nowait((frame_idx, fU))
            except Full:
                pass # 워커가 바쁘면 프레임을 건너뜀

        # 워커로부터 결과 수신 (논블로킹)
        try:
            _, kpsR, confR = q_out_ref.get_nowait()
            if kpsR is not None: lastR = {"kps": kpsR, "conf": confR}
        except Empty: pass
        try:
            _, kpsU, confU = q_out_user.get_nowait()
            if kpsU is not None: lastU = {"kps": kpsU, "conf": confU}
        except Empty: pass

        # 채점 로직 (do_score일 때 & 두 포즈가 모두 유효할 때)
        if do_score and (lastR["kps"] is not None) and (lastU["kps"] is not None):
            ...
            if s < 50.0: total_points = max(0, total_points - 1)
            elif s >= 70.0: total_points = min(100, total_points + 1)
            print(f"[total]@{total_points}")

        # 스켈레톤 표시
        if lastR["kps"] is not None: draw_pose(fR, lastR["kps"], kps_conf=lastR["conf"])
        if lastU["kps"] is not None: draw_pose(fU, lastU["kps"], kps_conf=lastU["conf"])
